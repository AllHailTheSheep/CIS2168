...
public class View {
     ...
     *
     * @param field  the field on the monster hunts the person
     */
    View(Object[][] field) {
        this.field = field;
     ...
     * Draws a monster.
     *
     * @param i  the row number of the monster
     * @param j  the column number of the monster
     */
    private void drawmonster(int i, int j) {
    ...
    private void drawCell(int row, int column) {
        ...
        else if (field[row][column] instanceof Monster) {
            drawmonster(row, column);
        }
    ...
    /**
     * Erases the monster and/or person from their old positions on the
     * screen and redraws them in their new positions. If the flag
     * <code>displayLookingAround</code> is true, will also show
     * in which directions the creature has looked.
     */
    void displayChanges() {
        // find new positions of monster and person
        for (int i = 0; i < numberOfRows; i++) {
            for (int j = 0; j < numberOfColumns; j++) {
                if (field[i][j] instanceof Monster) {
                
                    // possibly display monster looking around
                    if (displayLookingAround && !Model.isPersonsTurn) {
                        showLooks(monsterRow, monsterColumn, i, j);
                    }
                    
                    // if monster has moved, update display
                    if (i != monsterRow || j != monsterColumn) {
                        eraseCell(monsterRow, monsterColumn);
                        monsterRow = i;
                        monsterColumn = j;
                        drawmonster(i, j);
                    }
    ...
    /**
     * Shows what each creature looks at. This a bit complicated because
     * the creature has already moved by the time we do this, and we
     * want to show the "look" as having been performed before the
     * creature moved. Hence, <code>newRow</code> and <code>newColumn</code>
     * (which now contain the creature) are treated as empty cells.
     *
     * @param oldRow    The previous row of the creature (before moving)
     * @param oldColumn The previous column of the creature (before moving)
     * @param newRow    The current row of the creature (after moving)
     * @param newColumn The current column of the creature (after moving)
     */
    void showLooks(int oldRow, int oldColumn, int newRow, int newColumn) {
        ...
        for (int i = 0; i < looks.size(); i++) {
            ...
            while (legal(lookRow, lookColumn)) {
                ...
                // we want to pretend the creature move has not happened yet,
                // so new position of creature counts as an unoccupied cell
